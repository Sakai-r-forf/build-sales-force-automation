...
「企業データ削除」のNG企業一覧で確認・追加できます。社名と任意の公式ドメインをGCSの `ng_companies` に保存します。NG企業は取得時に除外し、既存企業も送信対象から外します。送信APIの予約時、クリック直前、遷移先・POST先の通信前に再確認します。記録の読み取りに失敗した場合も送信を止めます。

社名は全半角・空白・中点・株式会社などの法人格を正規化して照合します。登録名を含む支店名・ページタイトルも保守的に停止します。ドメインの配下も対象です。別名・別サイトは追加登録が必要です。自動解除はありません。削除済み企業は別欄に履歴を残します。

初期登録6社は `data/ng_companies_initial.json` に記録しています。実際の本番登録は明示的なデータ更新で行い、アプリ起動時にデータをリセットしません。
